 ...
 *
 * Forked from com.android.systemui.statusbar.notification.MediaNotificationProcessor.
 * Added a method that returns the calculated background color and formatted code.
 */
public class MediaNotificationProcessor {
    ...
    private static final float TEXT_COLOR_START_WIDTH_FRACTION = 0.4f;
     ...
     */
    public int processNotification(Notification notification, Notification.Builder builder) {
        ...
        Drawable drawable = largeIcon.loadDrawable(mPackageContext);
        if (notification.isColorizedMedia()) {
            ...
            paletteBuilder.setRegion(
                    (int) (bitmap.getWidth() * TEXT_COLOR_START_WIDTH_FRACTION),
                    0,
                    bitmap.getWidth(),
                    bitmap.getHeight());
            if (mFilteredBackgroundHsl != null) {
                paletteBuilder.addFilter((rgb, hsl) -> {
                    ...
                    return diff > DIFF_LOWER_BOUND && diff < DIFF_HIGHER_BOUND;
                });
            }
            ...
            backgroundColor = mContext.getColor(DEFAULT_BACKGROUND_COLOR);
        }
        Bitmap colorized = ImageGradientColorizer.colorize(drawable, backgroundColor,
